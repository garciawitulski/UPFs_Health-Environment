...
DEST <- file.path(OUT, "figures", "results_section")
dir.create(DEST, showWarnings = FALSE, recursive = TRUE)

col_ink   <- "#111827"
col_muted <- "#6B7280"
col_grid  <- "#E5E7EB"
col_men   <- "#1F3A4D"  # deep slate (matches panel C proportional family)
col_women <- "#6FA8B5"  # muted slate-teal (harmonises with YlGn heatmap in A)
col_men_soft   <- "#E1E7EB"
col_women_soft <- "#E6EEF1"
col_rule  <- "#1F2937"
...
# ---------------------------------------------------------------------------
t1_full <- read.csv(file.path(OUT, "Tabla1_UPF_por_energia_Argentina.csv"),
...
pal_r1 <- c("#F5F9FB", "#E1ECF0", "#C5D9DF", "#9CBDC7",
            "#6FA8B5", "#4A8896", "#336473", "#1F3A4D", "#102230")
...
save_pair(fig_r1, "Fig_R1_upf_heatmap", w_mm = 168, h_mm = 80)

# ---------------------------------------------------------------------------
# FIG R2: Bilateral attributable-deaths pyramid + PAF overlay (Table S10)
# ---------------------------------------------------------------------------
t2_full <- read.csv(file.path(OUT, "Tabla2_Muertes_atribuibles_Argentina.csv"),
                    check.names = FALSE, stringsAsFactors = FALSE)
...
# PAF lollipops placed well outside the bars, away from count labels
paf_max <- max(pyr$paf)
paf_span <- dmax * 0.42
pyr2 <- pyr2 %>%
...
x_limit <- (gap + dmax) + paf_span + dmax * 0.14  # room for PAF labels outside dots
...
fig_r2 <- ggplot(pyr2) +
  ...
  geom_rect(aes(xmin = xstart, xmax = xend,
                ...
                fill = sex), colour = NA) +
  # count labels at the outer end of each bar
  geom_text(aes(x = xend, y = yidx,
                label = comma(abs(round(deaths))),
                hjust = ifelse(sex == "Men", -0.15, 1.15)),
            size = 2.35, colour = "white", fontface = "bold") +
  # central age labels
  ...
  geom_text(aes(x = paf_x + ifelse(sex == "Men", -dmax * 0.035, dmax * 0.035),
                y = yidx,
                label = sprintf("%.0f%%", paf),
                hjust = ifelse(sex == "Men", 1, 0)),
            size = 1.9, colour = col_muted) +
  # bottom axis numbers
...
save_pair(fig_r2, "Fig_R2_attributable_pyramid", w_mm = 155, h_mm = 125)
...
save_pair(fig_r3, "Fig_R3_burden_lollipop", w_mm = 150, h_mm = 125)
...
# ---------------------------------------------------------------------------
sehs <- read.csv(file.path(OUT, "scenario_health_economic_summary.csv"),
...
save_pair(fig_r4, "Fig_R4_scenario_ladder", w_mm = 195, h_mm = 115)
...
pB_left  <- fig_r2 + labs(tag = "b") +
  theme(plot.margin = margin(6, -4, 4, -70)) + tag_theme
pB_right <- fig_r3 +
  theme(plot.margin = margin(6, 14, 4, 0))
pC <- fig_r4 + labs(tag = "c") +
  theme(legend.position = "top",
        plot.margin     = margin(4, 22, 4, 0)) + tag_theme

pB <- (pB_left | plot_spacer() | pB_right) +
  plot_layout(widths = c(1.15, 0.05, 1))

composite <- pA / pB / pC +
  plot_layout(heights = c(0.78, 1.35, 1.15))

save_pair(composite, "Fig_1_composite", w_mm = 220, h_mm = 285)
