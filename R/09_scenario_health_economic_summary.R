...
# Scenario-level deaths, YLL, life expectancy, and PVLE summaries reconstructed from the R pipeline.
...
earnings <- read_csv(file.path(data_dir, "EPH", "ingresos_por_estrato.csv"), show_col_types = FALSE)
...
scenario_labels <- c(
  ...
  red_20 = "20% reduction",
  red_50 = "50% reduction",
  ndg_NDG_conservative = "NDG conservative (target UPF=5.45%)",
  ndg_NDG_central = "NDG central (target UPF=3.63%)",
  ndg_NDG_strict = "NDG strict (target UPF=1.81%)"
)
...
write_csv(scenario_summary, file.path(out_dir, "scenario_health_economic_summary.csv"))
message("Guardado: ", file.path(out_dir, "scenario_health_economic_summary.csv"))
