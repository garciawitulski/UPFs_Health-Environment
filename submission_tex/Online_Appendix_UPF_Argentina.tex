...
\begin{itemize}
  \item NOVA processing class (1--4)
  \item energy density (kcal per 100\,g)
  \item environmental food group
        \textit{(Arrieta et~al.\ 2022; extended with OWID data)}
  \item impact coefficients per kg: GHG, land, water, eutrophication
\end{itemize}
};
...
\]
where $A_{sa}$ are UPF-attributable deaths from the comparative risk assessment model (Sections~B.2--B.4), and $L_a$ denotes standard remaining life expectancy at age group $a$ from the WHO reference table. \citep{Murray2003,WHO2024MethodsGBD,WHO2024DALY}
...
\clearpage
\input{table_s2_nova_classification_all_items.tex}
...
\vspace{0.8em}
\input{table_s7_scenario_composition.tex}

\clearpage
\vspace{0.8em}
\input{table_s8_environmental_coefficients.tex}
